...
def constrastLimit(image):
    ...
    channels = np.array(cv2.split(img_hist_equalized))
...
def removeSmallComponents(image, threshold):
    ...
    labelnum, labelimg, contours, GoCs = cv2.connectedComponentsWithStats(image, connectivity=8)
    sizes = contours[1:, -1]

    img2 = np.zeros(labelimg.shape, dtype=np.uint8)
    for label in range(1, labelnum):
        if sizes[label - 1] > threshold:
            img2[labelimg == label] = 255
...
def contourIsSign(perimeter, centroid, threshold):
    ...
    for p in perimeter:
        p = p[0]
        distance = np.sqrt(np.square(p[0] - centroid[0]) + np.square(p[1] - centroid[1]))
